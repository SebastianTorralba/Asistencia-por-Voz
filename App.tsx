            ...
            <div className="w-full max-w-2xl text-center space-y-8">
                ...
                {(step === 'idle' || step === 'recording') && (
                     <div className="flex justify-center">
                        ...
                        >
                            {step === 'recording' && <span className="absolute h-full w-full rounded-full bg-red-500 animate-ping opacity-75"></span>}
                            {step === 'recording' ? <StopIcon className="w-10 h-10 text-white" /> : <MicrophoneIcon className="w-10 h-10 text-white" />}
                        </button>
                    </div>
                )}
                
                {/* Step 3: Transcribe Controls */}
                {step === 'recorded' && audioBlob && (
                    <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-6 space-y-4">
                        <h3 className="text-lg font-semibold text-cyan-300">Grabación completa</h3>
                        <audio src={URL.createObjectURL(audioBlob)} controls className="w-full" />
                        <div className="flex justify-center gap-4">
                            <button onClick={resetState} className="px-4 py-2 text-sm font-medium text-gray-300 bg-gray-700/50 border border-gray-600 rounded-lg hover:bg-gray-700 transition-colors">Grabar de Nuevo</button>
                            <button onClick={handleTranscribe} className="px-6 py-2 text-sm font-medium text-white bg-cyan-500 rounded-lg hover:bg-cyan-600 transition-colors">Transcribir Audio</button>
                        </div>
                    </div>
                )}
                
                {/* Step 4: Generate List Controls */}
                {step === 'transcribed' && (
                     <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-6 space-y-4 text-left">
                        <h3 className="text-lg font-semibold text-cyan-300">Transcripción</h3>
                        <p className="text-gray-300 whitespace-pre-wrap bg-gray-900/50 p-3 rounded-md">{transcription}</p>
                         <div className="flex justify-center">
                            <button onClick={handleGenerateList} className="px-6 py-2 text-sm font-medium text-white bg-purple-500 rounded-lg hover:bg-purple-600 transition-colors">Generar Lista de Asistencia</button>
                         </div>
                    </div>
                )}
                    ...
                    <div className="w-full max-w-2xl text-left space-y-4">
                        <div className="flex justify-between items-center">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <button onClick={resetState} className="px-4 py-2 text-sm font-medium text-white bg-cyan-600 rounded-lg hover:bg-cyan-700 transition-colors">Empezar de Nuevo</button>
                            </div>
                        </div>
